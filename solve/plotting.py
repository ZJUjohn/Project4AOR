...
STD_TITLE_FONTSIZE = 14 # Increased
STD_LABEL_FONTSIZE = 12
STD_TICK_FONTSIZE = 10
STD_LEGEND_FONTSIZE = 10
...
def plot_q2_suitability_bubble_academic(df_q2_summary, q1_data_for_demand):
    ...
    if q1_data_for_demand.empty:
        print("Warning (Chart 3): Q1 data for demand is missing for bubble sizes. Using default size.")
        df_q2_summary['AnnualDemand_Total'] = 1000
    else:
        ...
        df_q2_summary['AnnualDemand_Total'] = np.maximum(df_q2_summary['AnnualDemand_Total'], 1)
    ...
    required_cols_q2 = [direct_cost_col, mm_cost_a_col, mm_cost_b_col, suitable_a_col, suitable_b_col, 'Customer_Location', 'AnnualDemand_Total']
    ...
    def determine_suitability_category_eng(row):
        s_a_text = str(row.get(suitable_a_col, "N/A")).strip()
    ...
    fig, ax = plt.subplots(figsize=(9.5, 6.5)) # Use standard size
    palette = {
    ...
    try:
        ...
        try: eq_line_idx = labels.index('Cost Equivalence Line (Direct = Multimodal)')
        except ValueError: eq_line_idx = -1

    if eq_line_idx != -1:
        handles = [handles[eq_line_idx]] + [h_val for i,h_val in enumerate(handles) if i != eq_line_idx]
        labels = [labels[eq_line_idx]] + [l_val for i,l_val in enumerate(labels) if i != eq_line_idx]

    ax.legend(handles, labels, title='Suitability & Demand Volume', fontsize=STD_LEGEND_FONTSIZE-1, loc=STD_LEGEND_LOC, bbox_to_anchor=STD_LEGEND_BBOX, borderaxespad=0.)
    ...
    fig.subplots_adjust(right=LEGEND_ADJUST_RIGHT -0.1, top=0.88)
...
def plot_q3_sankey_flows_plotly(df_q3_direct_vol, df_q3_coastal_vol, output_dir=PLOTS_DIR):
    ...
    try:
        ...
        y_pos_counters = {0.05: 0, 0.35: 0, 0.65: 0, 0.95: 0} # x_coord -> count of nodes for y spacing
...
if __name__ == '__main__':
    ...
    if not df_q3_summary.empty and pd.notna(df_q3_summary['TotalOptimalCost_Annual'].iloc[0]):
        plot_cost_volume_structure_academic(q1_annual_cost_val, df_q3_summary, df_q3_direct_vol, df_q1_details)
    ...
    if not df_q3_direct_vol.empty or not df_q3_coastal_vol.empty:
        plot_am_mode_split_q3_academic(df_q3_direct_vol, df_q3_coastal_vol)
    else: print("Skipping Chart 2 due to missing Q3 shipment data.")

    if not df_q2_summary.empty :
        plot_q2_suitability_bubble_academic(df_q2_summary, df_q1_details)
    ...
    if not df_q3_summary.empty:
        plot_kpi_summary_academic(df_q3_summary)
